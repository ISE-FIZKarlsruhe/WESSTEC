...
//		System.out.println(escapeCharacters(s));
		...
		for (int i = 0; i < itemEntries.getLength(); i++) {
			...
			String lang = escapeCharacters(itemNode.getAttribute("lang"));
			...
			if (content != null && !content.equals("")) fw.write("\tsioc:content\t\"" + content + "\"^^xsd:string;\n");
			...
			for (int j = 0; j < category.getLength(); j++) {
				...
				fw.write("\txlime:hasCategory [\n");
				fw.write("\t\txlime:hasConfidence\t\"" + hasConfidence + "\"^^xsd:double;\n");
				fw.write("\t\txlime:hasEntity\t<" + hasEntity.replaceAll("http://en.wikipedia.org/wiki/Category:","http://dbpedia.org/category/") + ">;\n");
				fw.write("\t] ;\n");
			}
			...
			for (int j = 0; j < annotations.getLength(); j++) {
				...
				for (int k = 0; k < mentions.getLength(); k++) {					
					...
					fw.write("\t\txlime:hasPostion [\n");
					fw.write("\t\t\txlime:hasStartPostion\t\"" + hasStartPosition + "\"^^xsd:long;\n");
					fw.write("\t\t\txlime:hasStopPostion\t\"" + (Long.parseLong(hasStartPosition) + Long.parseLong(hasLength)) + "\"^^xsd:long\n");
					fw.write("\t\t]\n");
				}
