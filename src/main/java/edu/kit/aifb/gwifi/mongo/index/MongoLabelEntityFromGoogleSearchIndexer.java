...
public class MongoLabelEntityFromGoogleSearchIndexer
{
	...
	private Wikipedia _zhwikipedia;
	private Wikipedia _enwikipedia;
	...
	public MongoLabelEntityFromGoogleSearchIndexer(String zhdbDir, String endbDir, String sLangLabel, String tLangLabel, String APIKey,
			String engineId, String year) throws Exception
	{
		...
		_zhwikipedia = new Wikipedia(new File(zhdbDir), false);
		_enwikipedia = new Wikipedia(new File(endbDir), false);
	...
	// "configs/MongoConfig_gwifi.properties" "configs/wikipedia-template-zh.xml" "configs/wikipedia-template-en.xml" "zh" "en" "" "" "2013"
		...
		while (curqueries.hasNext())
		{
			...
			if (curobjLabel == null)
			{
				...
				for (String link : links)
				{
					...
					Article article = _zhwikipedia.getArticleByTitle(title);
...
//		System.out.println(_googleForAllTextCollection);
		...
		while (curqueries.hasNext())
		{
			...
			{
				labelText = nlabelText.replace(" ", "_");
			}else
			{
...
//		_resultOfGoogleSearchCollection.createIndex(new BasicDBObject(DBConstants.LABEL_ENTITY_SOURCE, 1));
	...
	public void test(String labelText, List<String> links, String nlabelText)
	{
		...
		for (String link : links)
		{
			...
			if(IsChineseOrNot.isChinese(title))
			{
				article = _zhwikipedia.getArticleByTitle(title);
			}else
			{
				article = _enwikipedia.getArticleByTitle(title);
			}
			...
			if (article != null && article.getType().equals(PageType.article))
			{
				...
				if (!_sLang.equals(_tLang) && link.startsWith("https://" + _sLang.toString()))
				{
						String[] tIdAndEntity = _searcher.getCrossLingualEntity(sId).split(":");
						if (tIdAndEntity.length == 2)
